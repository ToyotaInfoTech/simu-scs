...
// The original code can be found at `external/inet/pyisicallayer/wireless/apsk/packetlevel/ApskRadio.cc`. 

#include "../../../inet/physicallayer/apskradio/SatelliteRadio.h"
...
void SatelliteRadio::initialize(int stage)
{
...
void SatelliteRadio::handleUpperPacket(Packet *packet)
{
...
void SatelliteRadio::sendUp(Packet *packet)
{
    if (protocol != nullptr && protocol != packet->getTag<PacketProtocolTag>()->getProtocol()) {
...
b SatelliteRadio::computePaddingLength(b length, const ConvolutionalCode *forwardErrorCorrection, const ApskModulationBase *modulation) const
{
...
const ApskModulationBase *SatelliteRadio::getModulation() const
{
...
void SatelliteRadio::encapsulate(Packet *packet) const
{
    auto phyHeader = makeShared<ApskPhyHeader>();
    phyHeader->setCrc(0);
...
void SatelliteRadio::decapsulate(Packet *packet) const
{
